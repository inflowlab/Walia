...
public struct WaliaObjCap has key {
    id: UID,
    cap: Cap,
    walrus_obj_id: ID,
}
...
public fun create_walia_obj_cap(ctx: &mut TxContext, cap: Cap, walrus_obj_id: ID) {
    let obj_cap = WaliaObjCap {
        id: object::new(ctx),
        cap,
        walrus_obj_id,
    };
    transfer::transfer(obj_cap, ctx.sender());
}
